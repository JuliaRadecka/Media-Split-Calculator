...
# -*- coding: utf-8 -*-
# Media Split Calculator — v5.4.4 (see chat for changelog)
...
# ---- App ----
st.set_page_config(page_title='📊 Media Split Calculator v5.4.4', layout='wide')
st.title('📊 Media Split Calculator — v5.4.4')
...
if st.session_state.mode == 'filters':
    ...
    if st.session_state.get('_pending_recalc', False):
        do_recalc(); st.rerun()  # force fresh tables after widget change
    ...
    if df_result is None or summary is None:
        ...
        cols = [c for c in ['placement','category','recommended budget','category priority','placement priority','minimum spend','maximum spend'] if c in df_result.columns]
        table_df = df_result[cols].copy()
        total_row = {c:'' for c in cols}
        if 'placement' in cols: total_row['placement']='ИТОГО'
        if 'recommended budget' in cols: total_row['recommended budget']=table_df['recommended budget'].sum()
    ...
    if uploaded is not None:
        try:
            ...
            st.info('📥 Файл загружен. Приоритеты категорий синхронизированы с текущими настройками.')
        except Exception as e:
