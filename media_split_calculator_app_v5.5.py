# -*- coding: utf-8 -*-
# Media Split Calculator — v5.4 (UI labels/layout patch only)
...
# ===== Main screen action buttons (before calculation) =====
# 9 equal columns; put Calculate in col[0], Upload in col[3]
cols9 = st.columns(9)
with cols9[0]:
    if st.button('Calculate', help='Run allocation now'):
        st.session_state.mode = 'calculate'
with cols9[3]:
    if st.button('Upload a file (.xlsx)', help='Go to edit mode and load a previous result (.xlsx)'):
...
if st.session_state.mode == 'calculate':
    ...
    # Two small buttons side-by-side, same visual weight as the edit button
    c1, spacer, c2 = st.columns([1,0.05,1])
    with c1:
        st.download_button('Download Results (CSV)', data=csv_bytes, file_name=csv_name, mime='text/csv')
    with c2:
        st.download_button('Download Results (.xlsx)', data=xls_bytes, file_name=xls_name,
                           mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    st.write("")
    if st.button('Edit Calculated Table'):
        st.session_state.mode = 'edit_result'
...
if st.session_state.mode == 'edit_result':
    ...
    with up_col:
        # accept only Excel, remove CSV mention in label
        uploaded = st.file_uploader('Upload a results file (.xlsx)', type=['xlsx'])
    if uploaded is not None:
        try:
            ...
            if loaded is None or 'placement' not in loaded.columns:
                st.error("В файле не найден лист 'Split by Placement' со столбцом 'placement'.")
            else:
    ...
    d1, d2 = st.columns(2)
    with d1:
        st.download_button('Download Result (CSV)', data=csv_b, file_name=csv_n, mime='text/csv', key='dl_edit_csv')
    with d2:
        st.download_button('Download Result (.xlsx)', data=xls_b, file_name=xls_n,
                           mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', key='dl_edit_xlsx')
