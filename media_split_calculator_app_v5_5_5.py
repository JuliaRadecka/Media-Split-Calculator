...
def apply_platform_bounds(df, bounds):
    """Platform Budget (min/max) on platform groups (platform_key), but
    only for rows, которые реально участвуют в основном распределении (main-пул):
    - category != 'other'
    - проходят gate-логику по category/placement priority (если она включена).
    Таким образом, платформенный min/max не тратится на служебные / free-float строки.
    """
    df = add_platform_key(df)
    df = df.copy()
    ...
    # определяем маску строк, которые могут участвовать в основном распределении
    cat_norm = df['category'].astype(str).map(normalize_category_key)
    other_mask = cat_norm == 'other'

    # есть ли приоритеты для включения gate-логики
    use_gates = bool(
        ('category priority' in df.columns and df['category priority'].notna().any()) or
        ('placement priority' in df.columns and df['placement priority'].notna().any())
    )
    if use_gates:
        cat_pr = pd.to_numeric(seriesize(df.get('category priority'), len(df)), errors='coerce').fillna(5.0)
        plc_pr = pd.to_numeric(seriesize(df.get('placement priority'), len(df)), errors='coerce').fillna(5.0)
        cat_gate = cat_pr <= 3
        free_gate = cat_pr >= 4
        gate_mask = ((cat_gate & (plc_pr <= 2)) | free_gate)
    else:
        gate_mask = pd.Series(True, index=df.index)

    main_mask = gate_mask & (~other_mask)
    ...
    for key, mm in (bounds or {}).items():
        ...
        # учитываем только строки, участвующие в main-пуле
        target = mask & main_mask
        ...
        if mn > 0:
            per_min = mn / n_rows
            current_min = pd.to_numeric(df.loc[target, 'minimum spend'], errors='coerce').fillna(0.0)
            df.loc[target, 'minimum spend'] = np.maximum(current_min, per_min)
        if mx > 0:
            per_max = mx / n_rows
            current_max = pd.to_numeric(df.loc[target, 'maximum spend'], errors='coerce').fillna(1e9)
            df.loc[target, 'maximum spend'] = np.minimum(current_max, per_max)
