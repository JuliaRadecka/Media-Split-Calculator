...
def _describe_goal_profile(selected_groups):
    ...
    if share_aw >= 0.6:
        return "🎯 Цель сплита: выбранные категории в большей степени про охват и узнаваемость (Awareness)."
    if share_conv >= 0.6:
        return "🎯 Цель сплита: выбранные категории в большей степени про перформанс и конверсии (Conversion)."
    return "🎯 Цель сплита: выбранные категории формируют сбалансированный профиль между Awareness и Conversion."
...
if st.session_state.mode == 'filters':
    ...
    uploaded = None
    if src_flag == 'upload':
        up_col, _, _, _ = st.columns([1, 3, 3, 3])
        with up_col:
