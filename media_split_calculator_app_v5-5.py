...
def ensure_mode():
    ...
    st.session_state.setdefault('edit_uploaded_parsed', False)
...
GOAL_BUCKETS = {
    ...
    "OLV PREM": "awareness",
    "PRG": "awareness",
    "Geoservices": "awareness",
    "Promopages": "awareness",
    "Bloggers": "awareness",
    "SOCIAL+TG": "balanced",
    "ECOM": "conversion",
    "MOB": "conversion",
...
def _describe_goal_profile(selected_groups):
    """Return human-readable description of goal focus (awareness / conversion / mixed)
    based on selected UI groups. Used for the helper banner in filters/result.
    """
    # Базовое описание профиля целей в зависимости от выбранных категорий
    if not selected_groups:
        base_msg = "🎯 Цель сплита: пока категории не выбраны — профиль целей будет смешанным по всему каталогу."
    else:
        cnt = {"awareness": 0, "balanced": 0, "conversion": 0}
        for g in selected_groups:
            bucket = GOAL_BUCKETS.get(g, "balanced")
            cnt[bucket] += 1
        total = max(cnt["awareness"] + cnt["balanced"] + cnt["conversion"], 1)
        share_aw = cnt["awareness"] / total
        share_conv = cnt["conversion"] / total

        if share_aw >= 0.6:
            base_msg = "🎯 Цель сплита: выбранные категории в большей степени про охват и узнаваемость (Awareness)."
        elif share_conv >= 0.6:
            base_msg = "🎯 Цель сплита: выбранные категории в большей степени про перформанс и конверсии (Conversion)."
        else:
            base_msg = "🎯 Цель сплита: выбранные категории формируют сбалансированный профиль между Awareness и Conversion."

    # Расширенная подсказка по тому, как типы категорий соотносятся с целями/стадиями воронки
    extra_msg = (
        "\n\n**Как категории соотносятся с целями сплита:**\n"
        "- **CTV** — Awareness. Premium-видео, широкое покрытие, повышение узнаваемости.\n"
        "- **OLV PREM (premium-видео/видео-инвентарь)** — Awareness или Engagement. Если это preroll/out-stream на широкую аудиторию — Awareness; если это контент с высокой досматриваемостью — Engagement.\n"
        "- **SOCIAL + TG** — Engagement / Consideration. Видео, посты, сторис вовлекают аудиторию и могут приводить трафик на сайт или промо.\n"
        "- **MOB** — Awareness / Engagement / Consideration / Conversion — зависит от инвентаря. Медийные баннеры ближе к Awareness, таргетированные/CPA-форматы — к Conversion.\n"
        "- **PRG (programmatic)** — может работать на любую стадию: Awareness / Consideration / Conversion в зависимости от закупаемого инвентаря и KPI.\n"
        "- **PROMOPAGES** — Engagement / Consideration. Промостраницы и лендинги помогают вовлечь, собрать данные и прогреть аудиторию.\n"
        "- **РСЯ / контекст** — Consideration / Conversion. Пользователь уже ищет продукт, мы попадаем в момент принятия решения — хорошо для трафика и продаж.\n"
        "- **ECOM** — Conversion. Карточки товаров, корзина, ретаргетинг и другие e‑commerce‑сценарии ведут к покупке или заказу.\n"
        "- **CPA, Geoperfom, перформанс-инвентарь** — Conversion. Перформанс-форматы оптимизированы под действия: лиды, заказы, конверсии.\n"
        "- **Direct** — Consideration / Conversion. Поисковые кампании по брендовым запросам ближе к Consideration/Conversion; медийные форматы зависят от конкретного инвентаря.\n"
    )

    return base_msg + extra_msg


def render_goal_profile_banner():
    """Render informational banner about goal profile based on selected categories."""
    groups = get_selected_ui_groups()
    msg = _describe_goal_profile(groups)
    st.info(msg)
...
def format_summary_banner(total_loaded, total_budget, other_share):
    """Summary banner: always show 100% of the scenario budget, without Free Float line."""
    return f"✅ Бюджет успешно распределён: {total_loaded:.2f} млн ₽ (100%)"
...
def get_selected_ui_groups():
    """Возвращает список выбранных UI-групп категорий.

    Сейчас используется один уровень выбора (cat_level1).
    Если он пуст (старые сценарии или импорт из старых файлов),
    используется cat_order.
    """
...
def filter_by_categories(df, picked_ui_groups):
    ...
    Сейчас используется один список категорий (cat_level1).
    Если он пуст, используется cat_order/picked_ui_groups (legacy-режим).
    ...
    # Новый режим: один уровень категорий
    lvl1 = st.session_state.get("cat_level1", []) or []
    has_levels = bool(lvl1)

    if has_levels:
        level_map = {}  # real_category(lower) -> level (всегда 1)
        for group in lvl1:
            for real in UI_GROUPS.get(group, [group]):
                norm_real = normalize_category_key(real)
                level_map[norm_real] = 1
        ...
        order_map = {cat: 1 for cat in keep_cats}
        # 'other' всегда в конец
        order_map["other"] = 2
        return df2, order_map

    # Legacy-режим: используем cat_order / picked_ui_groups
...
def apply_platform_bounds(df, bounds):
    """Platform Budget (min/max) по платформам (platform_key) поверх базовой логики 4.9.

    Важно:
    * Ограничения задаются на СУММУ бюджета по платформе (во всех выбранных категориях, кроме other).
    * Black List и фильтр категорий уже применены до вызова этой функции.
    * Здесь мы:
        - учитываем каталожные minimum/maximum spend;
        - расширяем gate для платформ с заданным бюджетом (чтобы строки точно участвовали);
        - раскладываем платформенный min/max по строкам пропорционально весам;
        - при невозможности выполнить ограничения полностью добавляем предупреждения (плашки).
    """
    ...
    in_edit = st.session_state.get('mode') == 'edit'

    # ❗Каждый расчёт — новый список предупреждений, чтобы плашки не копились.
    warnings = []

    # Гарантируем наличие колонок minimum/maximum
    if 'minimum spend' not in df:
        df['minimum spend'] = 0.0
    if 'maximum spend' not in df:
        df['maximum spend'] = 1e9

    # Нормализуем категорию и выделяем OTHER (free float)
    cat_norm = df['category'].astype(str).map(normalize_category_key)
    other_mask = cat_norm == 'other'

    # Базовые веса по формуле 4.9 (используем текущие alpha/beta)
    alpha = float(st.session_state.get('alpha_cache', 1.6))
    beta = float(st.session_state.get('beta_cache', 1.0))

    cp = _series_or_default(df, 'commercial priority', 0.25)
    plc_pr = _series_or_default(df, 'placement priority', 5.0)
    plc_safe = plc_pr.replace(0, 1.0)
    base_w = (cp ** alpha) * ((1.0 / plc_safe) ** beta)

    # ✅ Безопасно получаем общий бюджет сценария:
    raw_total = st.session_state.get('scenario_total_budget')
    if raw_total is None:
        raw_total = st.session_state.get('total_budget_cache', 240.0)
    try:
        total_budget_ref = float(raw_total or 0.0)
    except (TypeError, ValueError):
        total_budget_ref = 0.0

    # Проходим по платформам, для которых заданы min/max
    for key, mm in (bounds or {}).items():
        if not isinstance(mm, dict):
            continue

        user_min = float(mm.get('min', 0) or 0.0)
        user_max = float(mm.get('max', 0) or 0.0)
        # Если и min, и max не заданы – пропускаем
        if user_min <= 0 and user_max <= 0:
            continue

        # Флаг "пользователь поставил max < min" — для отдельной плашки
        user_inverted = (user_min > 0 and user_max > 0 and user_max < user_min)

        key_str = str(key).lower()
        canon = PLATFORM_CANON.get(key_str, None)
        platform_label = canon or key_str.upper()

        pk = df['platform_key'].astype(str).str.lower()
        pl = df['placement'].astype(str).str.lower()

        if canon is not None:
            mask = pk == canon.lower()
        else:
            mask = pk.str.contains(key_str, na=False)

        # также учитываем подстроку в raw placement (legacy-поведение)
        mask = mask | pl.str.contains(key_str, na=False)

        # работаем только с не-other строками (free float живёт отдельно)
        target = mask & (~other_mask)
        if not target.any():
            continue

        # Текущие каталожные границы по платформе (в рамках выбранных категорий)
        cur_min = pd.to_numeric(df.loc[target, 'minimum spend'], errors='coerce').fillna(0.0)
        cur_max = pd.to_numeric(df.loc[target, 'maximum spend'], errors='coerce').fillna(1e9)
        catalog_min_total = float(cur_min.sum())
        catalog_max_total = float(cur_max.sum())

        # Пользовательские границы по платформе
        plat_min = max(user_min, 0.0)
        plat_max = user_max if user_max > 0 else catalog_max_total

        # sanity-check с общим бюджетом сценария: min платформы не должен быть > total
        if total_budget_ref > 0 and plat_min > total_budget_ref:
            warnings.append(
                f"❗Бюджет по платформе {platform_label} в фильтрах превышает общий бюджет сценария "
                f"({plat_min:.2f} млн ₽ > {total_budget_ref:.2f} млн ₽). "
                f"Будет использован максимум допустимого бюджета сценария."
            )
            plat_min = total_budget_ref
            if plat_max > plat_min:
                plat_max = plat_min

        # Сопоставляем с каталогом и собираем предупреждения при конфликтах с каталогом
        if (not in_edit) and catalog_min_total > 0:
            # user min ниже каталожного минимума
            if 0 < plat_min < catalog_min_total:
                warnings.append(
                    f"❗В фильтрах минимальный бюджет по платформе {platform_label} ниже минимального бюджета каталога "
                    f"(минимум {plat_min:.2f} млн ₽ < минимум каталога {catalog_min_total:.2f} млн ₽). "
                    f"Будет использован минимум каталога."
                )
                plat_min = catalog_min_total
            # user max ниже каталожного минимума – придётся поднять до минимума
            if 0 < plat_max < catalog_min_total:
                warnings.append(
                    f"❗В фильтрах максимальный бюджет по платформе {platform_label} ниже минимального бюджета каталога "
                    f"(максимум {plat_max:.2f} млн ₽ < минимум каталога {catalog_min_total:.2f} млн ₽). "
                    f"Будет использован минимум каталога."
                )
                plat_max = catalog_min_total

        # user max выше каталожного максимума
        if (not in_edit) and catalog_max_total > 0 and plat_max > catalog_max_total:
            warnings.append(
                f"❗В фильтрах максимальный бюджет по платформе {platform_label} выше максимального бюджета каталога "
                f"(максимум {plat_max:.2f} млн ₽ > максимум каталога {catalog_max_total:.2f} млн ₽). "
                f"Будет использован максимум каталога."
            )
            plat_max = catalog_max_total

        # user min выше каталожного максимума
        if (not in_edit) and catalog_max_total > 0 and plat_min > catalog_max_total:
            warnings.append(
                f"❗В фильтрах минимальный бюджет по платформе {platform_label} выше максимального бюджета каталога "
                f"(минимум {plat_min:.2f} млн ₽ > максимум каталога {catalog_max_total:.2f} млн ₽). "
                f"Будет использован минимум каталога."
            )
            plat_min = catalog_max_total

        # ⚠️ Отдельный кейс: пользователь задал max < min
        if user_inverted and plat_max > 0:
            warnings.append(
                f"❗В фильтрах максимальный бюджет по платформе {platform_label} ниже минимального "
                f"(максимум {user_max:.2f} млн ₽ < минимум {user_min:.2f} млн ₽). "
                f"Диапазон скорректирован до {max(plat_min, plat_max):.2f} млн ₽."
            )
            v = max(plat_min, plat_max)
            plat_min = v
            plat_max = v
        else:
            # В остальных случаях, если после всех поправок min > max —
            # тихо поджимаем min до max БЕЗ дополнительной плашки.
            if plat_min > plat_max and plat_max > 0:
                plat_min = plat_max

        # Если после всех поправок диапазон всё равно вырожден и каталог нулевой – пропускаем
        if plat_min <= 0 and plat_max <= 0:
            continue

        # Раскладываем платформенный min/max по строкам пропорционально весам
        w = base_w[target].copy()
        if w.le(0).all():
            w = pd.Series(1.0, index=w.index)
        w_sum = float(w.sum()) if w.sum() > 0 else float(len(w))
        share = w / w_sum

        per_min = share * plat_min if plat_min > 0 else pd.Series(0.0, index=w.index)
        per_max = share * plat_max if plat_max > 0 else pd.Series(1e9, index=w.index)

        # Применяем к каталожным минимумам/максимумам с учётом ограничений
        new_min = np.maximum(cur_min.values, per_min.values)
        new_max = np.minimum(cur_max.values, per_max.values)

        # safety: new_min не должен быть > new_max
        over = new_min > new_max
        if np.any(over):
            new_min[over] = new_max[over]

        df.loc[target, 'minimum spend'] = new_min
        df.loc[target, 'maximum spend'] = new_max

        # Расширяем gate для этой платформы: placement priority не выше 2,
        # чтобы строки гарантированно попали в основной пул 4.9
        if 'placement priority' in df.columns:
            plc_sub = pd.to_numeric(
                seriesize(df.loc[target, 'placement priority'], len(df.loc[target])),
                errors='coerce'
            ).fillna(5.0)
            plc_sub = np.minimum(plc_sub, 2.0)
            df.loc[target, 'placement priority'] = plc_sub

    # Сохраняем НОВЫЙ список предупреждений в сессию для отображения в UI
    st.session_state['platform_budget_warnings'] = warnings
    return df
...
def apply_category_priorities(df, picked_ui_groups):
    """Присваиваем числовой приоритет категориям.

    Сейчас используется один уровень (cat_level1 → priority = 1).
    Если он не задан (legacy), используем порядок из cat_order / picked_ui_groups.
    ...
    if has_levels:
        # Все выбранные группы получают приоритет 1
        for group in lvl1:
            for real in UI_GROUPS.get(group, [group]):
...
def export_excel(df_split, df_sum, base_df_with_ids):
    ...
    # Опциональные фильтры для выгрузки в _meta
    platform_bounds = st.session_state.get('platform_bounds', {})
    blacklist = st.session_state.get('bl_selected', [])
    try:
        pb_json = json.dumps(platform_bounds, ensure_ascii=False)
    except Exception:
        pb_json = "{}"
    bl_str = ",".join(blacklist) if blacklist else ""
...
# ---- App ----
st.set_page_config(page_title='📊 Media Split Calculator v5_5', layout='wide')
st.title('📊 Media Split Calculator — v5_5')
st.markdown(
    f"""
    <style>
...
.stFileUpload [data-testid="stFileUploadDropzone"] button[kind="secondary"]::before {{
    content: "Загрузить файл";
}}
.stFileUpload [data-testid="stFileUploadDropzone"] button[kind="secondary"] > div {{
    color: transparent;
}}
/* Скрываем стандартный текст в зоне загрузки и добавляем русскую подпись */
.stFileUpload [data-testid="stFileUploadDropzone"] > div:first-child > div {{
    opacity: 0;
}}
.stFileUpload [data-testid="stFileUploadDropzone"] > div:first-child::before {{
    content: "Загрузите файл .xlsx. Вес файла до 200 МБ";
    position: absolute;
    left: 50%;
...
if st.session_state.mode != 'edit':
    ...
    with st.container(border=True):
        ...
        btn_label = "⬆️ Свернуть" if st.session_state.get("show_optional_params") else "⬇️ Опциональные настройки"
        st.button(
            btn_label,
    ...
    if st.session_state.get("show_optional_params", False):
        with st.container(border=True):
            st.markdown("### ⚙️ Опциональные параметры")

            st.markdown("**Бюджет платформы**")
            ...
            cat_col, bl_col = st.columns(2)

            with cat_col:
                st.markdown("**Выбор категорий**")
            ...
            with bl_col:
                st.markdown("**Исключить платформы (Black List)**")
...
if st.session_state.mode == 'filters':
    render_goal_profile_banner()

if st.session_state.mode == 'filters':
    a,b = st.columns(2)
    with a:
        if st.button('🧮 Рассчитать'): do_recalc(); st.session_state.mode='result'; st.rerun()
    with b:
        if st.button('📂 Загрузить файл (.xlsx)'):
            st.session_state['edit_source'] = 'upload'
            st.session_state['base_df'] = None
            st.session_state['df_result'] = None
            st.session_state['calculated_df'] = None
            st.session_state['summary'] = None
            st.session_state['edit_uploaded_parsed'] = False
    ...
    if df_result is None or summary is None:
        ...
        render_goal_profile_banner()
        for w in st.session_state.get('platform_budget_warnings', []) or []:
            st.warning(w)

        st.subheader('📈 Рекомендуемое распределение')
        ...
        st.data_editor(table_df, use_container_width=True, hide_index=True, disabled=True,
                       ...
                       'placement priority': 'Приоритет платформы',
                       'minimum spend': 'Минимальный бюджет',
                       'maximum spend': 'Максимальный бюджет',
                       'recommended budget': st.column_config.NumberColumn('Рекомендуемый бюджет', format='%.6f')
                       },
                       column_order=column_order,
                       key='result_table_v550')

        st.subheader('📊 Доля по категориям')
        ...
        if not sum_df.empty:
            ...
            sum_df = sum_df.rename(columns={
                'category': 'Категория',
                'recommended budget': 'Рекомендуемый бюджет',
                'share_%': 'Доля_%',
            })
        ...
        with ccsv: st.download_button('💾 Скачать файл (CSV)', data=export_csv(df_result), file_name='split_by_placement.csv', mime='text/csv')
        with cxlsx: st.download_button('💾 Скачать файл (.xlsx)', data=export_excel(df_result, st.session_state.get('summary', pd.DataFrame()), st.session_state.base_df),
                                       file_name='media_split_results.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        if st.button('✏️ Редактировать расчет'):
            st.session_state['edit_source'] = 'result'
            st.session_state.mode = 'edit'
            st.rerun()

elif st.session_state.mode == 'edit':
    st.subheader('✏️ Редактировать расчет')
    src_flag = st.session_state.get('edit_source')

    uploaded = None
    if src_flag == 'upload':
        up_col, _, _, _ = st.columns([3, 3, 3, 3])
        with up_col:
    ...
    def _restore_cat_order_from_meta(meta_df):
        ...
        try:
            ...
            pass
        return False
    def _restore_optional_from_meta(meta_df):
        """Восстанавливаем опциональные фильтры из _meta:
        - platform_bounds (JSON)
        - blacklist (строка через запятую)
        Также синхронизируем поля y_min/y_max/... с platform_bounds,
        чтобы UI показывал те же значения, что используются в расчёте.
        """
        try:
            # platform_bounds
            pb_series = seriesize(meta_df.get('platform_bounds', pd.Series(dtype='object')))
            raw_pb = pb_series.iloc[0] if len(pb_series) > 0 else None
            pb = {}
            if isinstance(raw_pb, str) and raw_pb.strip():
                try:
                    pb = json.loads(raw_pb)
                except Exception:
                    pb = {}
            if isinstance(pb, dict):
                st.session_state['platform_bounds'] = pb

                def _get_pb(p, field):
                    return float(pb.get(p, {}).get(field, 0.0) or 0.0)

                st.session_state['y_min']  = _get_pb('yandex', 'min')
                st.session_state['y_max']  = _get_pb('yandex', 'max')
                st.session_state['da_min'] = _get_pb('da', 'min')
                st.session_state['da_max'] = _get_pb('da', 'max')
                st.session_state['vk_min'] = _get_pb('vk', 'min')
                st.session_state['vk_max'] = _get_pb('vk', 'max')
                st.session_state['mts_min'] = _get_pb('mts', 'min')
                st.session_state['mts_max'] = _get_pb('mts', 'max')

            # blacklist
            bl_series = seriesize(meta_df.get('blacklist', pd.Series(dtype='object')))
            raw_bl = bl_series.iloc[0] if len(bl_series) > 0 else None
            if isinstance(raw_bl, str) and raw_bl.strip():
                st.session_state['bl_selected'] = [x.strip() for x in raw_bl.split(',') if x.strip()]
        except Exception:
            # В случае любой ошибки ничего не трогаем
            pass
    if uploaded is not None and not st.session_state.get('edit_uploaded_parsed', False):
        try:
            ...
            if '_meta' in xls.sheet_names:
                ...
                _restore_cat_order_from_meta(meta_sheet)
                _restore_optional_from_meta(meta_sheet)
                # restore numeric settings from _meta where available
                try:
                    alpha_vals = pd.to_numeric(seriesize(meta_sheet.get('alpha'), len(meta_sheet)), errors='coerce').dropna()
                    if len(alpha_vals) > 0:
                        st.session_state['alpha_cache'] = float(alpha_vals.iloc[0])
                except Exception:
                    pass
                try:
                    beta_vals = pd.to_numeric(seriesize(meta_sheet.get('beta'), len(meta_sheet)), errors='coerce').dropna()
                    if len(beta_vals) > 0:
                        st.session_state['beta_cache'] = float(beta_vals.iloc[0])
                except Exception:
                    pass
                try:
                    ff_vals = pd.to_numeric(seriesize(meta_sheet.get('free_float_%'), len(meta_sheet)), errors='coerce').dropna()
                    if len(ff_vals) > 0:
                        st.session_state['other_share_cache'] = float(ff_vals.iloc[0])
                except Exception:
                    pass
                try:
                    tb_vals = pd.to_numeric(seriesize(meta_sheet.get('total_budget'), len(meta_sheet)), errors='coerce').dropna()
                    if len(tb_vals) > 0:
                        st.session_state['total_budget_cache'] = float(tb_vals.iloc[0])
                except Exception:
                    pass
                try:
            ...
            st.session_state['edit_banner_text'] = (
                f"📥 Бюджет успешно подгружен: {total_loaded:.2f} млн ₽ (100%)"
            )
            ...
            st.session_state['edit_uploaded_parsed'] = True
    ...
    if waiting_for_file:
        if st.button('⬅️ Назад'): st.session_state['edit_source']=None; st.session_state.mode='filters'; st.rerun()
        st.stop()
    ...
    # Goal profile banner in Edit mode (between upload button and budget banner)
    render_goal_profile_banner()
    ...
    if cur_df is not None and cur_sum is not None:
        d1,d2 = st.columns(2)
        with d1: st.download_button('💾 Скачать файл (CSV)', data=export_csv(cur_df), file_name='split_by_placement.csv', mime='text/csv')
        with d2: st.download_button('💾 Скачать файл (.xlsx)', data=export_excel(cur_df, cur_sum, st.session_state.base_df),
