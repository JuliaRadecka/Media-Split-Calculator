...
GOAL_DETAILS = {
    "CTV": (
        "CTV — преимущественно **Awareness**: премиальное видео, широкое покрытие, "
        "рост узнаваемости бренда."
    ),
    "OLV PREM": (
        "OLV PREM — **Awareness** или **Engagement**: preroll / out-stream и другое "
        "premium‑видео дают охват, форматы с высокой досматриваемостью усиливают "
        "вовлечение."
    ),
    "SOCIAL+TG": (
        "SOCIAL+TG — **Engagement / Consideration**: посты, видео и сторис дают "
        "вовлечение и могут приводить трафик на сайт или промо‑страницы."
    ),
    "MOB": (
        "MOB — от **Awareness** до **Conversion**: баннеры и медийка работают на охват, "
        "таргетированные и CPA‑форматы — на действия и конверсии."
    ),
    "PRG": (
        "PRG (programmatic) — любой уровень воронки: результат зависит от закупаемого "
        "инвентаря и KPI кампании (Awareness / Consideration / Conversion)."
    ),
    "Promopages": (
        "Promopages — **Engagement / Consideration**: промо‑лендинги для вовлечения, "
        "сбора данных и прогрева аудитории."
    ),
    "Geoservices": (
        "Geoservices (Geomedia / Geoperfom) — чаще **Awareness** или **Conversion** в "
        "зависимости от форматов и настроек геотаргетинга."
    ),
    "ECOM": (
        "ECOM — **Conversion**: карточки, корзина, ретаргетинг и другие форматы, "
        "которые напрямую подталкивают к покупке или заказу."
    ),
    "Bloggers": (
        "Bloggers — в первую очередь **Awareness / Engagement**: нативные интеграции, "
        "которые повышают узнаваемость и доверие к бренду."
    ),
    "Direct": (
        "Direct / контекст — **Consideration / Conversion**: перехватывает уже "
        "сформированный спрос в момент поиска и помогает довести до действия."
    ),
}

def _describe_goal_profile(selected_groups):
    """Формирует текст плашки про цели сплита (Awareness / Conversion / mixed)
    и дополняет его расшифровкой по выбранным группам категорий.
    """
    if not selected_groups:
        return (
            "🎯 Цель сплита: пока категории не выбраны — профиль целей будет "
            "смешанным по всему каталогу."
        )

    # 1) Считаем общий «сдвиг» профиля (Awareness / Conversion / Balanced)
    ...
    if share_aw >= 0.6:
        main_msg = (
            "🎯 Цель сплита: выбранные категории в большей степени про охват и "
            "узнаваемость (Awareness)."
        )
    elif share_conv >= 0.6:
        main_msg = (
            "🎯 Цель сплита: выбранные категории в большей степени про "
            "перформанс и конверсии (Conversion)."
        )
    else:
        main_msg = (
            "🎯 Цель сплита: выбранные категории формируют "
            "сбалансированный профиль между Awareness и Conversion."
        )

    # 2) Добавляем расшифровку по тем группам, которые реально выбраны
    details = [GOAL_DETAILS[g] for g in selected_groups if g in GOAL_DETAILS]
    if not details:
        return main_msg

    bullets = "\n".join(f"• {text}" for text in details)
    return main_msg + "\n\n**Как читаются выбранные категории:**\n" + bullets
