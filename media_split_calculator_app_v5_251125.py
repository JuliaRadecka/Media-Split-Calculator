...
def apply_platform_bounds(df, bounds):
    ...
    * Здесь мы:
    ...
    # Инициализируем список предупреждений
    warnings = st.session_state.get('platform_budget_warnings', [])
    if warnings is None:
        warnings = []
    ...
    # Общий бюджет сценария для sanity-check по платформенному min
    raw_tb = st.session_state.get('scenario_total_budget')
    if raw_tb is None:
        raw_tb = st.session_state.get('total_budget_cache', 240.0)
    try:
        total_budget_ref = float(raw_tb)
    ...
    # Сохраняем предупреждения в сессию для отображения в UI
