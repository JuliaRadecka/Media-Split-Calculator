...
def allocate_budget(df, total_budget=240.0, alpha=1.6, beta=1.0, other_share=10.0, use_gates=True):
    df = df.copy()
...
# --- Priority gates toggle (as in v4.9 by default) ---
use_gates = st.checkbox('Учитывать пороги приоритетов (cat ≤ 3 и plc ≤ 2)', value=True, key='use_gates_checkbox')
st.session_state['use_gates'] = bool(use_gates)
        ...
        with c1:
            if st.button('✅ Сохранить и пересчитать'):
                ...
                new_df_result, new_summary, new_margin = allocate_budget(
                    ...
                    use_gates=bool(st.session_state.get('use_gates', True))
                )
