...
# -*- coding: utf-8 -*-
# media_split_calculator_app_v5.4_final.py
...
def apply_platform_bounds(df, bounds):
    """Row-level min/max setters by fuzzy placement match (yandex/da/vk/mts)."""
    df = df.copy()
...
def allocate_budget(df, total_budget=240.0, alpha=1.6, beta=1.0, other_share=10.0, use_gates=None):
    ...
    # Auto-gate: like 4.9 when priorities exist; else gates off
    ...
    # Split budget: "OTHER" vs main (fixed percentage)
    ...
    if remaining < -1e-9:
        # Scale mins proportionally to fit (blue info message)
    ...
    # Normalize exactly to main_budget
...
for name in all_placements:
    ...
    opts.append(label);  defs.append(label) if marked else None
    ...
    if (base is None or (hasattr(base, 'empty') and base.empty)) and (current is None or current.empty):
        ...
        with c1:
            if st.button('✅ Сохранить и пересчитать'):
                ...
                # Keep only edited rows, preserve order
                try:
                ...
                # Blue info if mins scaled
                if meta.get('scaled_mins'):
                    coef = meta.get('scale_coef', 1.0)
                    st.info(f'Мinimum spend был пропорционально масштабирован коэффициентом {coef:.4f}, чтобы уложиться в бюджет.')

        with c2:
            if st.button('⬅️ Отмена'):
                st.session_state.mode = 'calculate'

        # Downloads from the *current edited result*
...
if st.session_state.mode == 'calculate':
    ...
    # Warn if min could not be applied (no rows left for the group after filters)
    for key in requested_groups.keys():
        mask = df_to_use['placement'].astype(str).str.lower().str.contains(key)
        if not mask.any():
            # Orange warning
            label = key.upper()
            st.warning(f"Для платформы {label} после фильтров не осталось площадок; group min не применён.")
    ...
    if meta.get('scaled_mins'):
        st.info(f'Мinimum spend был пропорционально масштабирован коэффициентом {meta.get("scale_coef",1.0):.4f}, чтобы уложиться в бюджет.')

    # Order by chosen categories
...
# ---- Bottom downloads & Edit button (for current calc)
