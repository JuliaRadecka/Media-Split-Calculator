...
all_cats = ['CPA', 'CTV', 'Direct', 'ECOM', 'Geomedia', 'Geoperfom', 'Media', 'MOB', 'OLV PREM', 'PREM', 'PRG', 'Promopages', 'SOCIAL', 'THEM', 'РСЯ']
...
if st.session_state.mode == 'edit_result':
    ...
    if st.session_state.get('last_recalc_text'):
        ...
        # Quick downloads right after recalc (edit mode)
        try:
            _df_cur = st.session_state.get('df_result')
            _sum_cur = st.session_state.get('summary')
            if _df_cur is not None and _sum_cur is not None and not _df_cur.empty:
                _csv_bytes = _df_cur.to_csv(index=False).encode('utf-8')
                ...
                with d1:
                    st.download_button('💾 Download Result (CSV)',
                                       ...
                                       mime='text/csv',
                                       key='dl_calc_csv_edited')
                with d2:
                    from io import BytesIO
                    _xls = BytesIO()
                    try:
                        with pd.ExcelWriter(_xls, engine='xlsxwriter') as writer:
                            _df_cur.to_excel(writer, index=False, sheet_name='Placement split')
                            _sum_cur.to_excel(writer, index=False, sheet_name='Summary by category')
    ...
    if (base is None or (hasattr(base, 'empty') and base.empty)) and (current is None or current.empty):
        ...
        with c1:
            if st.button('✅ Сохранить и пересчитать'):
                ...
                allocated = float(new_df_result['recommended budget'].fillna(0).sum())
                percent = min(100.0, (allocated / float(total_budget) * 100.0)) if total_budget > 0 else 0.0
