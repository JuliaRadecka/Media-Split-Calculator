    ...
    if (base is None or (hasattr(base, 'empty') and base.empty)) and (current is None or current.empty):
        ...
        with c1:
            if st.button('✅ Сохранить и пересчитать'):
                ...
                # 4) Пересчитываем распределение
                new_df_result, summary = allocate_budget(base_df,
                    total_budget=st.session_state['total_budget'],
                    free_float_share=st.session_state.get('free_float_share',10),
                    alpha=st.session_state.get('alpha',1.0),
                    beta=st.session_state.get('beta',1.0),
                    category_filters=st.session_state.get('category_filters',[]),
                    platform_limits=st.session_state.get('platform_limits',{}))
