...
# -*- coding: utf-8 -*-
# Media Split Calculator — v5.4.1
# Changes:
#  • Filters mode: no auto-calc on widget changes; only by "🧮 Calculate"
#  • Result mode: optional filters auto-recalc via lightweight callbacks that set a flag
#  • No st.rerun() inside callbacks (Streamlit auto-reruns after callback)
#  • Removed default/session_state double init for widgets (no yellow warnings)
...
def ensure_mode():
    ...
    if '_pending_recalc' not in st.session_state:
...
# --- App ---
st.set_page_config(page_title='📊 Media Split Calculator v5.4.1', layout='wide')
st.title('📊 Media Split Calculator — v5.4.1')
...
def mark_for_recalc():
    # Callback only sets a flag; Streamlit will rerun automatically
...
# ---------- Controls (keys-only; no manual state writes) ----------
if st.session_state.mode != 'edit':
    ...
    with p1:
        st.caption('Yandex')
        st.number_input('min (mln ₽)', key='y_min',
                        value=float(st.session_state.get('y_min', 0.0)),
                        step=10.0, on_change=mark_for_recalc)
        st.number_input('max (mln ₽)', key='y_max',
                        value=float(st.session_state.get('y_max', 0.0)),
                        step=10.0, on_change=mark_for_recalc)
    with p2:
        st.caption('DA')
        st.number_input('min (mln ₽)', key='da_min',
                        value=float(st.session_state.get('da_min', 0.0)),
                        step=10.0, on_change=mark_for_recalc)
        st.number_input('max (mln ₽)', key='da_max',
                        value=float(st.session_state.get('da_max', 0.0)),
                        step=10.0, on_change=mark_for_recalc)
    with p3:
        st.caption('VK')
        st.number_input('min (mln ₽)', key='vk_min',
                        value=float(st.session_state.get('vk_min', 0.0)),
                        step=10.0, on_change=mark_for_recalc)
        st.number_input('max (mln ₽)', key='vk_max',
                        value=float(st.session_state.get('vk_max', 0.0)),
                        step=10.0, on_change=mark_for_recalc)
    with p4:
        st.caption('MTS')
        st.number_input('min (mln ₽)', key='mts_min',
                        value=float(st.session_state.get('mts_min', 0.0)),
                        step=10.0, on_change=mark_for_recalc)
        st.number_input('max (mln ₽)', key='mts_max',
                        value=float(st.session_state.get('mts_max', 0.0)),
                        step=10.0, on_change=mark_for_recalc)
    ...
    st.multiselect('Exclude placements from calculation', options=all_placements,
                   key='bl_selected', on_change=mark_for_recalc)

    st.markdown('---')

# ---------- Modes ----------
if st.session_state.mode == 'filters':
    # In filters mode, do NOT auto-calc even if _pending_recalc is set
    cA, cB = st.columns(2)
    with cA:
    ...
    # If widgets changed, auto-recalc here
