...
# -*- coding: utf-8 -*-
# Media Split Calculator — v5.4.6
# Fixes over v5.4.5:
# - Removed non-ASCII literal inside f-string that caused "unterminated string literal" on some Windows setups.
# - Using explicit \u25A1 (white square) via a variable, avoiding nested quotes.
# - Keeps all functional changes from v5.4.5 (banners, toggle for advanced columns, robust upload & recalc).
...
def apply_category_priorities_from_order(df, order):
    ...
    df['category priority'] = df['category'].astype(str).str.lower().map(m).fillna(df.get('category priority', 5.0))
    return df
...
# ---- App ----
st.set_page_config(page_title='📊 Media Split Calculator v5.4.6', layout='wide')
st.title('📊 Media Split Calculator — v5.4.6')
...
if st.session_state.mode != 'edit':
    ...
    def _toggle_cat(cat_key):
        ...
        default_symbol = '\u25A1'  # safe white square
        # build prefix using str() to avoid nested quotes issues across environments
        prefix_val = ord_show.get(label, default_symbol)
        prefix = f"{prefix_val}  "
    ...
    for i, cat in enumerate(cats):
        ...
        default_symbol = '\u25A1'
        prefix = f"{ord_show.get(cat, default_symbol)}  "
...
if st.session_state.mode == 'filters':
    ...
    if st.session_state.get('_pending_recalc', False):
        do_recalc(); st.rerun()  # ensure fresh tables
    ...
    if uploaded is not None:
        try:
            ...
            rb = pd.to_numeric(base_df.get('recommended budget', pd.Series(dtype='float64')), errors='coerce').fillna(0)
            total_loaded = float(rb.sum())
