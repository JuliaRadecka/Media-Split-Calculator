...
# -*- coding: utf-8 -*-
# Media Split Calculator — v5.6 (fixed & consolidated)
# Changes vs v5.5/v5.6:
# • Stable 3-mode routing: filters → result → edit (Back returns to result).
# • No use of deprecated st.experimental_rerun (now st.rerun).
# • Editor applies changes by __id (persistent row ids) — no merge-by-text.
# • Exports:
#     - CSV/Excel exclude: commercial priority, W, available, __id.
#     - Excel adds sheet "_meta" with parameters + __id↔placement/category.
# • Black List options cleaned from None/NaN and kept in st.session_state.bl_selected.
# • Edit screen downloads the *edited* state only; Result screen downloads the *first* calc.
# • After Save & Recalculate in Edit, margin is shown just like on the result screen.
# • Button sizes on the start screen left as default (no extra stretching).
#
...
def ensure_unique_ids(df):
    ...
    if '__id' not in df.columns:
        df['__id'] = np.arange(1, len(df) + 1, dtype=np.int64)
    return df

def apply_editor_to_base(base_df, edited_df, editable_cols):
    """Apply only editable columns by integer __id (no string merges)."""
...
def export_excel(df_split, df_sum, base_df_with_ids):
    ...
    with pd.ExcelWriter(_xls, engine='openpyxl') as writer:
        ...
        # mapping __id↔placement/category
...
# show as plain names (no checkboxes inside "pills")
...
if st.session_state.mode == 'result':
    ...
    if df_result is None or summary is None:
        ...
        table_df = df_result[show_cols].copy()
        total_row = {col: '' for col in show_cols}
...
if st.session_state.mode == 'edit':
    ...
    # If user uploads an Excel produced by this app — reconstruct editor state
    ...
    if base_df is None:
        ...
        with c1:
            if st.button('💾 Save & Recalculate'):
                ...
                st.session_state.summary = summary.copy()
                st.session_state.total_margin = float(total_margin)
                st.success(f'💰 Общая маржинальность сплита: {float(total_margin):.2f}%')
        with c2:
            if st.button('⬅️ Back'):
                set_mode('result')
