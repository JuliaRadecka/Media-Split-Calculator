...
def _stable_id_row(placement, category):
...
def ensure_unique_ids(df):
    df = df.copy()
    if '__id' not in df.columns or df['__id'].isna().all():
        df['__id'] = [ _stable_id_row(p,c) for p,c in zip(df.get('placement'), df.get('category')) ]
    return df
...
def export_csv(df):
    drop_cols = [c for c in ['commercial priority','W','available','__id'] if c in df.columns]
    return df.drop(columns=drop_cols, errors='ignore').to_csv(index=False).encode('utf-8')

def export_excel(df_split, df_sum, base_df_with_ids):
    x = BytesIO()
    with pd.ExcelWriter(x, engine='openpyxl') as w:
        drop_cols = [c for c in ['commercial priority','W','available','__id'] if c in df_split.columns]
        df_split.drop(columns=drop_cols, errors='ignore').to_excel(w, index=False, sheet_name='Split by Placement')
        df_sum.to_excel(w, index=False, sheet_name='Summary by Category')
        meta = pd.DataFrame([collect_meta()])
        meta.to_excel(w, index=False, sheet_name='_meta')
        start_row = len(meta)+2
        id_map = base_df_with_ids[['__id','placement','category']].copy()
        id_map.to_excel(w, index=False, sheet_name='_meta', startrow=start_row)
    x.seek(0)
    return x.getvalue()
...
def apply_blacklist(df, bl):
    if not bl: return df.copy()
    return df[~df['placement'].isin(bl)].copy()

def apply_platform_bounds(df, bounds):
    df = df.copy()
    if 'minimum spend' not in df.columns: df['minimum spend']=0.0
    if 'maximum spend' not in df.columns: df['maximum spend']=1e9
    for key,mm in (bounds or {}).items():
        ...
        if mn>0: df.loc[mask,'minimum spend']=mn
        if mx>0: df.loc[mask,'maximum spend']=mx
    return df
...
def _ensure_other_summary(summary_df, other_budget):
    try:
        ...
        if 'other' not in set(cats):
            summary_df = pd.concat([summary_df, pd.DataFrame([{'category':'other','recommended budget':float(other_budget)}])], ignore_index=True)
        return summary_df
    except Exception:
        return summary_df
...
def allocate_budget(df, total_budget=240.0, alpha=1.6, beta=1.0, other_share=10.0, use_gates=None):
    ...
    for col, default in [('commercial priority',0.25),('category priority',5.0),('placement priority',5.0),('minimum spend',0.0),('maximum spend',1e9)]:
    ...
    if df_main.empty:
        ...
        empty_summary = pd.DataFrame({'category':pd.Series(dtype='object'),'recommended budget':pd.Series(dtype='float'),'share_%':pd.Series(dtype='float')})
    ...
    for _ in range(160):
        ...
        remaining = main_budget - df_main['recommended budget'].sum()
    s = df_main['recommended budget'].sum()
    if s>0: df_main['recommended budget'] = (df_main['recommended budget']/s)*main_budget
...
st.set_page_config(page_title='📊 Media Split Calculator v5.6', layout='wide')
st.title('📊 Media Split Calculator — v5.6')
...
platform_bounds = {'yandex':{'min':y_min,'max':y_max}, 'da':{'min':da_min,'max':da_max}, 'vk':{'min':vk_min,'max':vk_max}, 'mts':{'min':mts_min,'max':mts_max}}
...
if 'cat_order' not in st.session_state: st.session_state.cat_order = []
...
for i, cat in enumerate(all_cats):
    ...
    cat_cols[i].checkbox(prefix+cat, key=key, value=(cat in st.session_state.cat_order), on_change=_toggle_cat, args=(key,))
...
if 'bl_selected' not in st.session_state: st.session_state.bl_selected = []
    ...
    with cA:
        if st.button('🧮 Calculate'):
            ...
            if order_map and 'category' in df_result.columns:
                ...
                df_result = df_result.sort_values(by=['_cat_ord','recommended budget'], ascending=[True, False]).drop(columns=['_cat_ord'])
            ...
            st.session_state.success_msg = f"Бюджет успешно распределён: {float(st.session_state.total_budget_cache):.2f} млн ₽ (100%)"
            set_mode('result')
    with cB:
        if st.button('✏️ Upload a file (.xlsx)'):
            set_mode('edit')
...
if st.session_state.mode == 'result':
    ...
    if df_result is None or summary is None:
        ...
        visible = [c for c in ['placement','category','recommended budget'] if c in df_result.columns]
        additional = [c for c in ['category priority','placement priority','minimum spend','maximum spend'] if c in df_result.columns]
        table_df = df_result[visible + additional].copy()
        ...
        cfg = {col: st.column_config.NumberColumn(col, format='%.6f') if col=='recommended budget' else st.column_config.Column(col) for col in table_df.columns}
        for col in additional:
            try: cfg[col] = st.column_config.Column(col, hidden=True)
            except TypeError: cfg[col] = st.column_config.Column(col)
        st.data_editor(table_df, use_container_width=True, hide_index=True, disabled=True, column_config=cfg, column_order=visible+additional)
        ...
        st.dataframe(sum_df.round(2), use_container_width=True, hide_index=True)
        if total_margin is not None:
            st.markdown(f"### 💰 Общая маржинальность сплита: **{float(total_margin):.2f}%**")
        col_csv, col_xlsx = st.columns(2)
        with col_csv:
            st.download_button('💾 Download Results (CSV)', data=export_csv(df_result), file_name='split_by_placement.csv', mime='text/csv')
        with col_xlsx:
            st.download_button('💾 Download Results (.xlsx)', data=export_excel(df_result, summary, st.session_state.base_df), file_name='media_split_results.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
...
if st.session_state.mode == 'edit':
    ...
    if base_df is not None and current_df is not None and 'recommended budget' in current_df.columns:
        if '__id' in base_df.columns and '__id' in current_df.columns:
            cur_rb = current_df[['__id','recommended budget']].copy()
            base_df = base_df.merge(cur_rb, on='__id', how='left', suffixes=('','_cur'))
            base_df['recommended budget'] = base_df['recommended budget_cur'].combine_first(base_df.get('recommended budget'))
            base_df = base_df.drop(columns=[c for c in base_df.columns if c.endswith('_cur')], errors='ignore')
        else:
            cur = current_df.copy()
            cur['_key'] = cur['placement'].map(_norm_text)+'|'+cur['category'].map(_norm_text)
            base_df['_key'] = base_df['placement'].map(_norm_text)+'|'+base_df['category'].map(_norm_text)
            base_df = base_df.merge(cur[['_key','recommended budget']], on='_key', how='left', suffixes=('','_cur'))
            base_df['recommended budget'] = base_df['recommended budget_cur'].combine_first(base_df.get('recommended budget'))
            base_df = base_df.drop(columns=['_key','recommended budget_cur'], errors='ignore')
            ...
            if '_meta' in xls.sheet_names:
                ...
                try:
                    ...
                    if {'__id','placement','category'}.issubset(id_map.columns):
                        ...
                        base_df = ensure_unique_ids(base_df) if base_df['__id'].isna().any() else base_df
                    else:
                        base_df = ensure_unique_ids(split_df)
                except Exception:
                    base_df = ensure_unique_ids(split_df)
            else:
                ...
                cur['_key'] = cur['placement'].map(_norm_text)+'|'+cur['category'].map(_norm_text)
                base_df = cur.merge(split_df[['_key','category priority','placement priority','minimum spend','maximum spend','recommended budget']], on='_key', how='left', suffixes=('','_edited'))
                base_df = ensure_unique_ids(base_df.drop(columns=['_key']))
            st.session_state.base_df = base_df.copy()
            st.success('Файл загружен. Состояние редактора восстановлено.')
        except Exception as e:
            st.error(f'Не удалось прочитать файл: {e}')
    if st.session_state.get('edit_save_attempted') and base_df is None:
        st.info('Сначала выполните расчёт (Calculate) или загрузите Excel с _meta.')
    if base_df is not None:
        ...
        column_cfg = {'__id': st.column_config.Column('__id', hidden=True, disabled=True),
                      'recommended budget': st.column_config.NumberColumn('recommended budget', format='%.6f')}
        edited = st.data_editor(editor_df, use_container_width=True, num_rows='fixed', disabled=['placement','category'],
                                column_config=column_cfg, column_order=show_cols, key='editor_table_v56')
        c1, c2 = st.columns(2)
        with c1:
            if st.button('✅ Save & Recalculate'):
                st.session_state.edit_save_attempted = True
                ...
                df_result, summary, total_margin, meta = allocate_budget(df_in,
                    total_budget=float(st.session_state.total_budget_cache),
                    alpha=float(st.session_state.alpha_cache),
                    beta=float(st.session_state.beta_cache),
                    other_share=float(st.session_state.other_share_cache),
        ...
        if cur_df is not None and cur_sum is not None:
            d1, d2 = st.columns(2)
            with d1:
                st.download_button('💾 Download Result (CSV)', data=export_csv(cur_df), file_name='split_by_placement.csv', mime='text/csv')
            with d2:
