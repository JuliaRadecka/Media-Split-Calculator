# -*- coding: utf-8 -*-
# Media Split Calculator — v5.6 (patched per latest specs)
...
def _stable_id_row(placement, category):
    ...
    # 64-bit deterministic id from sha1 to stay compact but stable across sessions/files
    h = hashlib.sha1(key.encode("utf-8")).hexdigest()
    return int(h[:16], 16)  # 64-bit slice
...
def ensure_unique_ids(df):
    """Assign stable deterministic __id by placement|category hash.
    If __id already present — keep it (for backward compatibility)."""
    df = df.copy()
...
def export_excel(df_split, df_sum, base_df_with_ids):
    ...
    with pd.ExcelWriter(_xls, engine='openpyxl') as writer:
        export_df = df_split.drop(columns=[c for c in ['commercial priority','W','available','__id'] if c in df_split.columns], errors='ignore')
...
if st.session_state.mode == 'result':
    ...
    if df_result is None or summary is None:
        ...
        if st.session_state.get('success_msg'):
            st.success(st.session_state['success_msg'])  # success banner
        st.subheader('📈 Recommended Split by Placement')
        # Show only three columns by default; other columns are present but hidden (via data_editor, user can toggle)
        ...
        # Build a view with all columns but order the primary three first
        ...
        # Use data_editor in read-only mode to get column visibility "eye" control
        ...
        # Mark additional columns hidden by default (if supported by current Streamlit; harmless otherwise)
        for col in additional:
            try:
                cfg[col] = st.column_config.Column(col, hidden=True)
            except TypeError:
        ...
        st.data_editor(
            ...
            disabled=True,
            column_config=cfg,
            column_order=visible + additional
        )
...
if st.session_state.mode == 'edit':
    ...
    # Pre-fill "recommended budget" from last result so user edits an actual result (not None)
    ...
    # Do NOT show info banner preemptively; only if user tries to save with empty base
