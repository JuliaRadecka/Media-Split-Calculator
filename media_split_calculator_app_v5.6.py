...
# -*- coding: utf-8 -*-
# Media Split Calculator — v5.6 (patched to spec)
# See header in previous attempt for detailed changelog.
...
if st.session_state.mode == 'filters':
    ...
    with cA:
        if st.button('🧮 Calculate', use_container_width=True):
            ...
            set_mode('result')
    with cB:
        if st.button('✏️ Upload a file (.xlsx)', use_container_width=True):
            set_mode('edit')
...
if st.session_state.mode == 'result':
    ...
    if df_result is None or summary is None:
        ...
        col_csv, _, col_xlsx = st.columns([1,0.1,1])
        with col_csv:
            st.download_button('💾 Download Results (CSV)',
                               data=export_csv(df_result),
                               file_name='split_by_placement.csv', mime='text/csv', use_container_width=True)
        with col_xlsx:
        ...
        if st.button('✏️ Edit Calculated Table', use_container_width=True):
            set_mode('edit')
