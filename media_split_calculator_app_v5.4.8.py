...
# -*- coding: utf-8 -*-
# Media Split Calculator — v5.4.8
# Changes over v5.4.7:
# 1) Upload (Edit): percent in blue banner is computed from file's own total (Summary by Category, if present),
#    not from the current slider. Also, if found, we sync the slider to that total for consistency.
# 2) Extra guardrails to eliminate "'float' object has no attribute 'fillna'" in all branches.
# 3) Result table: small hint explains that the "eye" menu shows only rendered columns; toggle adds service columns.
...
def apply_category_priorities_from_order(df, order):
    ...
    if 'category priority' in df.columns:
        fallback = pd.to_numeric(df['category priority'], errors='coerce')
        mapped = mapped.where(~mapped.isna(), fallback)
    mapped = mapped.fillna(5.0)
...
# ---- App ----
st.set_page_config(page_title='📊 Media Split Calculator v5.4.8', layout='wide')
st.title('📊 Media Split Calculator — v5.4.8')
...
if st.session_state.mode == 'filters':
    ...
    if df_result is None or summary is None:
        ...
        st.caption("Подсказка: «глазик» показывает/скрывает только те колонки, которые уже отрисованы. Включите тумблер, чтобы добавить служебные колонки в таблицу, а затем управляйте ими через меню «глазика».")

        st.subheader('📈 Recommended Split by Placement')
        base_cols = ['placement','category','recommended budget']
        adv_cols  = [c for c in ['category priority','placement priority','minimum spend','maximum spend'] if c in df_result.columns]
        show_advanced = st.toggle('Показать служебные колонки', value=False, key='result_adv_cols')
        cols = base_cols + (adv_cols if show_advanced else [])
        table_df = df_result[cols].copy()
        total_row = {c:'' for c in cols}
        total_row['placement'] = 'ИТОГО'; total_row['recommended budget']=table_df['recommended budget'].sum()
        ...
        with cxlsx: st.download_button('💾 Download Results (.xlsx)', data=export_excel(df_result, summary, st.session_state.base_df),
    ...
    def _restore_cat_order_from_meta(meta_df):
        try:
            raw = meta_df.get('selected_categories', pd.Series(dtype='object')).iloc[0]
        ...
        try:
            ...
            if 'Summary by Category' in xls.sheet_names:
                try:
                    ...
                    if 'recommended budget' in sum_df_file.columns:
                        total_from_summary = float(pd.to_numeric(sum_df_file['recommended budget'], errors='coerce').fillna(0).sum())
                except Exception:
                    total_from_summary = None
            ...
            if 'recommended budget' in base_df.columns:
                rb_series = pd.to_numeric(base_df['recommended budget'], errors='coerce').fillna(0)
            else:
                rb_series = pd.Series(dtype='float64')
            ...
            # sync slider to file total if we have a solid reference
    ...
    with c1:
        if st.button('🔄 Save & Recalculate'):
            ...
            st.session_state.summary = summary.copy()
            st.session_state.total_margin = float(total_margin)
            total_loaded = float(pd.to_numeric(df_res.get('recommended budget'), errors='coerce').fillna(0).sum())
            tb = float(st.session_state.get('total_budget_cache', total_loaded))
            pct = (total_loaded/tb*100.0) if tb>0 else 100.0
            other = float(st.session_state.get('other_share_cache', 10.0))
            st.success(f"✅ Бюджет успешно распределён: {total_loaded:.2f} млн ₽ ({pct:.0f}%), Free Float Share ({other:.0f}%)")
