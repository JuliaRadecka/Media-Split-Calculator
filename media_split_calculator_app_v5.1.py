...
if st.session_state.mode == 'edit_result':
    ...
    if df_res is None:
        ...
        if uploaded is not None:
            try:
                if uploaded.name.lower().endswith('.csv'):
                    ...
                    # Пытаемся прочитать листы по именам, если их нет — берём первый
                    try:
        ...
        with c1:
            if st.button('💾 Apply Edits'):
                ...
                # Пересчёт сводки и маржи из отредактированной таблицы
                new_summary, new_margin = recompute_from_result(st.session_state.df_result, float(st.session_state.get('total_budget_cache', 0) or 0) or float(240.0))
    ...
    # Показать обновлённый результат (после правок/загрузки)
    ...
    dl1, dl2 = st.columns([1, 1])  # кнопки в одну линию
    with dl1:
