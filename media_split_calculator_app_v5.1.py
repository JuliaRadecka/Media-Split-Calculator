...
if st.session_state.mode == 'edit_result':
    ...
    base = st.session_state.get('last_calc_base')  # исходные данные последнего расчёта
    ...
    if (base is None or (hasattr(base, 'empty') and base.empty)) and (current is None or current.empty):
        ...
        # Оставляем только редактируемые поля
        allowed_cols = ['placement','category','category priority','placement priority','minimum spend','maximum spend']
        view_df = (current if current is not None else base)[[c for c in allowed_cols if c in (current if current is not None else base).columns]].copy()

        st.caption('Скорректируйте приоритеты и min/max. После сохранения я пересчитаю Recommended budget.')
        edited = st.data_editor(view_df, use_container_width=True, num_rows='dynamic')

        c1, c2 = st.columns([1,1])
        with c1:
            if st.button('💾 Apply Edits (Recalculate)'):
                # Берём полный базовый набор, подменяем отредактированные поля по ключу placement
                full_base = base.copy() if base is not None else None
                if full_base is None and current is not None:
                    # fallback: воссоздадим базу из текущего результата
                    full_base = current.copy()
                if 'placement' in edited.columns and full_base is not None:
                ...
                # Пересчёт на полном наборе с учётом правок
                new_df_result, new_summary, new_margin = allocate_budget(
                    full_base[[c for c in full_base.columns if c in set(allowed_cols + ['recommended budget'])]],
                ...
                st.session_state.last_calc_base = full_base  # чтобы дальнейшие правки продолжались от этой базы
    ...
    \1
        # cache base input for later re-calculation
        st.session_state.last_calc_base = df_input.copy() if 'df_input' in locals() else (df.copy() if 'df' in locals() else None),
        alpha=float(alpha),
        beta=float(beta),
...
# Выполняем ТОЛЬКО когда мы в режиме расчёта и df_result уже существует
if st.session_state.mode == 'calculate' and 'df_result' in locals():
    # CSV — только Placement split (как и в 4.9)
    csv_bytes = df_result.to_csv(index=False).encode('utf-8')

    # Один ряд: CSV | Excel | Edit | Back — с очень узкими зазорами
    col_csv, gap1, col_xlsx, gap2, col_edit, gap3, col_back = st.columns([1, 0.02, 1, 0.02, 1, 0.02, 1])
    ...
    with col_xlsx:
        # Excel — два листа: Placement split и Summary by category
        xls = BytesIO()
        try:
