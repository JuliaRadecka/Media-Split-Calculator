...
with p1:
    st.caption('Yandex')
    y_min = st.number_input('min (mln ₽)', key='y_min', value=0.0, step=10.0, help='Минимальный бюджет для Yandex')
    y_max = st.number_input('max (mln ₽)', key='y_max', value=0.0, step=10.0, help='Максимальный бюджет для Yandex')
with p2:
    st.caption('DA')
    da_min = st.number_input('min (mln ₽)', key='da_min', value=0.0, step=10.0, help='Минимальный бюджет для DA')
    da_max = st.number_input('max (mln ₽)', key='da_max', value=0.0, step=10.0, help='Максимальный бюджет для DA')
with p3:
    st.caption('VK')
    vk_min = st.number_input('min (mln ₽)', key='vk_min', value=0.0, step=10.0, help='Минимальный бюджет для VK')
    vk_max = st.number_input('max (mln ₽)', key='vk_max', value=0.0, step=10.0, help='Максимальный бюджет для VK')
with p4:
    st.caption('MTS')
    mts_min = st.number_input('min (mln ₽)', key='mts_min', value=0.0, step=10.0, help='Минимальный бюджет для MTS')
    mts_max = st.number_input('max (mln ₽)', key='mts_max', value=0.0, step=10.0, help='Максимальный бюджет для MTS')
            ...
            try:
                if uploaded.name.lower().endswith('.csv'):
                    ...
                    try:
                        ...
                        df_res = pd.read_excel(uploaded)  # первый лист
        ...
        with c1:
            if st.button('💾 Apply Edits'):
                ...
                new_summary, new_margin = recompute_from_result(
                    ...
                    float(st.session_state.get('total_budget_cache', 240.0))
                )
