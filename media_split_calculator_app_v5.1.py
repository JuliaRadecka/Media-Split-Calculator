...
if st.session_state.mode == 'edit_result':
    ...
    if (base is None or (hasattr(base, 'empty') and base.empty)) and (current is None or current.empty):
        ...
        right_sp, right_btn = st.columns([6,1])
        with right_btn:
            from io import BytesIO
            _xls = BytesIO()
            try:
                ...
                with pd.ExcelWriter(_xls, engine='openpyxl') as writer:
                    view_df.to_excel(writer, index=False, sheet_name='Edited table')
            _xls.seek(0)
            st.download_button('Download as Excel', data=_xls.getvalue(), file_name='edited_table.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
