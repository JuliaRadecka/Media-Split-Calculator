...
def _describe_goal_profile(selected_groups):
    ...
    if not selected_groups:
        return "🎯 Цель сплита: пока категории не выбраны — профиль целей будет смешанным по всему каталогу."
    ...
    share_bal = cnt["balanced"] / total
    share_conv = cnt["conversion"] / total

    # Небольшие доли других стадий тоже подсвечиваем, чтобы профиль целей был понятнее.
    minor_threshold = 0.05  # от 5% считаем, что стадия уже "заметна" в сплите

    if share_aw >= 0.6:
        # Основной фокус — Awareness, но при необходимости подсвечиваем дополнительные стадии
        extras = []
        if share_bal > minor_threshold:
            extras.append("доля форматов на Consideration")
        if share_conv > minor_threshold:
            extras.append("доля форматов на Conversion")

        extra_text = ""
        if extras:
            extra_text = " При этом в сплите также присутствует " + " и ".join(extras) + "."

        return (
            "🎯 Цель сплита: выбранные категории в большей степени про охват и узнаваемость "
            "(Awareness)." + extra_text
        )

    if share_conv >= 0.6:
        # Основной фокус — Conversion, но показываем, если есть Awareness/Consideration
        extras = []
        if share_aw > minor_threshold:
            extras.append("доля форматов на Awareness")
        if share_bal > minor_threshold:
            extras.append("доля форматов на Consideration")
        ...
        return (
            "🎯 Цель сплита: выбранные категории в большей степени про перформанс и конверсии "
            "(Conversion)." + extra_text
        )

    # Смешанный профиль без ярко выраженного перекоса
    return (
        "🎯 Цель сплита: выбранные категории формируют смешенный сплит "
