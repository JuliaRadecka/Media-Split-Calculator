...
# -*- coding: utf-8 -*-
# Media Split Calculator — v5.4.0 (keys-only state + auto-recalc callbacks + margin banner on upload + robust Free Float restore)
...
def apply_editor_to_base(base_df, edited_df, editable_cols):
    ...
    patch = patch.dropna(subset=['__id'])
    patch = patch.drop_duplicates(subset='__id', keep='last')
    merged = base.merge(patch, on='__id', how='left', suffixes=('', '__edit'))
    for col in editable_cols:
        col_e = col + '__edit'
        if col_e in merged.columns:
            merged[col] = merged[col_e].combine_first(merged[col])
            merged.drop(columns=[col_e], inplace=True)
    return merged
...
def margin_from_current_budgets(df) -> float:
    df = df.copy()
    if 'recommended budget' not in df.columns:
        return 0.0
    rb = pd.to_numeric(df['recommended budget'], errors='coerce').fillna(0)
    cp = pd.to_numeric(df.get('commercial priority', 0.25), errors='coerce').fillna(0.25)
    df2 = pd.DataFrame({'rb': rb, 'cp': cp})
    df2 = df2[df2['rb'] > 0]
    if df2.empty:
        return 0.0
    return float((df2['rb'] * df2['cp']).sum() / df2['rb'].sum() * 100.0)
...
# --- Recalc callback (for optional filters) ---
def recalc_callback():
    # Only auto-recalc in filters/result
    if st.session_state.get('mode') not in ('filters', 'result'):
        return
    df0 = src_df.copy()
    # Compose platform bounds from current state
    platform_bounds = st.session_state.get('platform_bounds', {})
    ...
    st.session_state.base_df = ensure_stable_ids(df_result.copy())
    st.session_state.mode = 'result'
    st.rerun()

# --- Controls (keys-only; no manual assignments) ---
if st.session_state.mode != 'edit':
    ...
    with col2:
        st.slider('α — Agency Profit Weight', 1.0, 2.5,
                  float(st.session_state.get('alpha_cache', 1.6)), 0.1, key='alpha_cache')
    with col3:
        st.slider('β — Client Priority Weight', 0.5, 2.0,
                  float(st.session_state.get('beta_cache', 1.0)), 0.1, key='beta_cache')
    with col4:
        st.slider('Free Float Share (%)', 0.0, 30.0,
                  float(st.session_state.get('other_share_cache', 10.0)), 1.0, key='other_share_cache')

    st.markdown('**Platform Budget (mln ₽, min/max) — optional**')
    p1, p2, p3, p4 = st.columns(4)
    with p1:
        st.caption('Yandex')
        st.number_input('min (mln ₽)', key='y_min', value=float(st.session_state.get('y_min', 0.0)),
                        step=10.0, on_change=recalc_callback)
        st.number_input('max (mln ₽)', key='y_max', value=float(st.session_state.get('y_max', 0.0)),
                        step=10.0, on_change=recalc_callback)
    with p2:
        st.caption('DA')
        st.number_input('min (mln ₽)', key='da_min', value=float(st.session_state.get('da_min', 0.0)),
                        step=10.0, on_change=recalc_callback)
        st.number_input('max (mln ₽)', key='da_max', value=float(st.session_state.get('da_max', 0.0)),
                        step=10.0, on_change=recalc_callback)
    with p3:
        st.caption('VK')
        st.number_input('min (mln ₽)', key='vk_min', value=float(st.session_state.get('vk_min', 0.0)),
                        step=10.0, on_change=recalc_callback)
        st.number_input('max (mln ₽)', key='vk_max', value=float(st.session_state.get('vk_max', 0.0)),
                        step=10.0, on_change=recalc_callback)
    with p4:
        st.caption('MTS')
        st.number_input('min (mln ₽)', key='mts_min', value=float(st.session_state.get('mts_min', 0.0)),
                        step=10.0, on_change=recalc_callback)
        st.number_input('max (mln ₽)', key='mts_max', value=float(st.session_state.get('mts_max', 0.0)),
                        step=10.0, on_change=recalc_callback)
    ...
    def _toggle_cat(cat_key):
        ...
        # Trigger the same recalc after category toggle
        recalc_callback()
    ...
    st.multiselect('Exclude placements from calculation', options=all_placements,
                   default=st.session_state.bl_selected, key='bl_selected', on_change=recalc_callback)
...
if st.session_state.mode == 'filters':
    ...
    with cA:
        if st.button('🧮 Calculate'):
            # Manual run (same pipeline as in auto-recalc)
            recalc_callback()
    with cB:
    ...
    # --- Restore Free Float and banner correctly on upload ---
    def _restore_free_float(xls, split_df):
        # Priority 1: _meta.free_float_% or free_float_share
        try:
            if '_meta' in xls.sheet_names:
                meta_sheet = pd.read_excel(xls, '_meta')
                for col in ['free_float_%', 'free_float_share']:
                    if col in meta_sheet.columns and pd.notna(meta_sheet[col].iloc[0]):
                        val = float(meta_sheet[col].iloc[0])
                        st.session_state['other_share_cache'] = val
                        return val
        except Exception:
            pass
        # Priority 2: Summary by Category (row 'other')
        try:
            if 'Summary by Category' in xls.sheet_names:
                sum_df = pd.read_excel(xls, 'Summary by Category')
                # Try to use share_% if present; else recompute
                if 'category' in sum_df.columns:
                    cats = sum_df['category'].astype(str).str.lower()
                    mask = cats == 'other'
                    if mask.any():
                        if 'share_%' in sum_df.columns and pd.notna(sum_df.loc[mask, 'share_%']).any():
                            val = float(sum_df.loc[mask, 'share_%'].iloc[0])
                            st.session_state['other_share_cache'] = val
                            return val
                        if 'recommended budget' in sum_df.columns:
                            other_sum = float(pd.to_numeric(sum_df.loc[mask, 'recommended budget'], errors='coerce').fillna(0).sum())
                            total_sum = float(pd.to_numeric(sum_df['recommended budget'], errors='coerce').fillna(0).sum())
                            if total_sum > 0:
                                val = other_sum / total_sum * 100.0
                                st.session_state['other_share_cache'] = val
                                return val
        except Exception:
            pass
        # Priority 3: compute from split_df
        try:
            rb = pd.to_numeric(split_df.get('recommended budget', pd.Series(dtype='float64')), errors='coerce').fillna(0)
            total = float(rb.sum())
            other_mask = split_df['category'].astype(str).str.lower() == 'other'
            other = float(pd.to_numeric(split_df.loc[other_mask, 'recommended budget'], errors='coerce').fillna(0).sum())
            if total > 0:
                val = other / total * 100.0
                st.session_state['other_share_cache'] = val
                return val
        except Exception:
            pass
        return float(st.session_state.get('other_share_cache', 10.0))
    ...
    if uploaded is not None:
        try:
            ...
            # Banner + margin + restored Free Float
            restored_ff = _restore_free_float(xls, split_df)
            rb_series = pd.to_numeric(base_df.get('recommended budget', pd.Series(dtype='float64')), errors='coerce').fillna(0)
            total_loaded = float(rb_series.sum())
            tb = float(st.session_state.get('total_budget_cache', 0))
            pct = (total_loaded / tb * 100.0) if tb > 0 else 0.0
            st.info(f"✅ Бюджет успешно подгружен: {total_loaded:.2f} млн ₽ ({pct:.0f}%), Free Float Share ({restored_ff:.0f}%)")
            st.markdown(f"### 💰 Общая маржинальность сплита: **{margin_from_current_budgets(base_df):.2f}%**")
        except Exception as e:
