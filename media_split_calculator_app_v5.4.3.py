...
# -*- coding: utf-8 -*-
# Media Split Calculator — v5.4.3
# Fix: In Edit mode after "Save & Recalculate" show allocated sum from fresh result,
#      so banner reflects the actual recompute (and never 0 if cache isn't set).
# Base: v5.4.2 behavior (Filters only by button; Result auto-recalc; no rerun in callbacks).
...
# --- App ---
st.set_page_config(page_title='📊 Media Split Calculator v5.4.3', layout='wide')
st.title('📊 Media Split Calculator — v5.4.3')
...
if st.session_state.mode == 'filters':
    ...
    with c1:
        if st.button('🔄 Save & Recalculate'):
            ...
            # Compute allocated sum from fresh result for banner
            total_loaded = float(pd.to_numeric(df_result.get('recommended budget'), errors='coerce').fillna(0).sum())
            tb = float(st.session_state.get('total_budget_cache', total_loaded))
            pct = (total_loaded / tb * 100.0) if tb > 0 else 100.0
            st.success(f"✅ Бюджет успешно распределён: {total_loaded:.2f} млн ₽ ({pct:.0f}%)")
            st.markdown(f"### 💰 Общая маржинальность сплита: **{float(total_margin):.2f}%**")
    with c2:
        if st.button('⬅️ Back'):
