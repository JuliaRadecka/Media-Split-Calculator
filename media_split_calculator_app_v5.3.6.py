# -*- coding: utf-8 -*-
# Media Split Calculator — v5.3.6 (edit-fixes: carry budgets to Edit, dedup patch, blue info after upload, margin banner on upload)
# Based on v5.3.5
...
def apply_editor_to_base(base_df, edited_df, editable_cols):
    # Robust "merge" by __id that tolerates duplicates in the edited view
    base = base_df.copy()
...
def margin_from_current_budgets(df) -> float:
    ...
    if 'recommended budget' not in df.columns:
        return 0.0
    df['rb'] = pd.to_numeric(df['recommended budget'], errors='coerce')
    df['cp'] = pd.to_numeric(df.get('commercial priority', 0.25), errors='coerce').fillna(0.25)
    df = df[df['rb'].notna() & (df['rb'] > 0)]
    if df.empty:
        return 0.0
    return float((df['rb'] * df['cp']).sum() / df['rb'].sum() * 100.0)
...
if st.session_state.mode != 'edit':
    ...
    # Platform bounds, categories, blacklist would be here (omitted for brevity in this compact patch sample)
...
if st.session_state.mode == 'filters':
    ...
    with cA:
        if st.button('🧮 Calculate'):
            ...
            df0 = src_df.copy()
            # NOTE: apply optional filters/bounds here in your full file
            ...
            # >>> carry budgets into editor source so Edit sees them immediately
    ...
    if df_result is None or summary is None:
        ...
        # downloads omitted in compact patch
    ...
    if uploaded is not None:
        try:
            ...
            # blue info banner with totals and free-float
            total_loaded = float(pd.to_numeric(base_df.get('recommended budget'), errors='coerce').fillna(0).sum())
            tb = float(st.session_state.get('total_budget_cache', 0))
            pct = (total_loaded / tb * 100.0) if tb > 0 else 0.0
            ff = float(st.session_state.get('other_share_cache', 0))
            st.info(f"✅ Бюджет успешно подгружен: {total_loaded:.2f} млн ₽ ({pct:.0f}%), Free Float Share ({ff:.0f}%)")
            # margin banner for uploaded data (from current numbers, not recompute core)
    ...
    # if came from result and already have result budgets, ensure editor sees them
    if base_df is None and current_df is not None:
        st.session_state.base_df = ensure_stable_ids(current_df.copy())
        base_df = st.session_state.base_df

    if base_df is None:
        base_df = ensure_stable_ids(src_df.copy())
        st.session_state.base_df = base_df.copy()
