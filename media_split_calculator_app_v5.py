...
# v5: фильтры + улучшения UI (CSV/Excel, ИТОГО, скрытые колонки для "глазика")
...
if st.session_state.mode == 'edit':
    ...
    show_cols = ['placement', 'category', 'recommended budget',
                 'category priority', 'placement priority', 'minimum spend', 'maximum spend']
    available_cols = [c for c in show_cols if c in df_result.columns]
    ...
    col_cfg = {}
    for extra in ['category priority', 'placement priority', 'minimum spend', 'maximum spend']:
        if extra in available_cols:
            col_cfg[extra] = st.column_config.NumberColumn(label=extra, help="Доп. колонка", format="%.2f", hidden=True)
    if 'recommended budget' in available_cols:
        col_cfg['recommended budget'] = st.column_config.NumberColumn(label='recommended budget', format="%.2f")

    st.dataframe(table_df, use_container_width=True, column_config=col_cfg)

    st.subheader('📊 Summary by Category')
    sum_df = summary.copy()
    tot = {'category': 'ИТОГО', 'recommended budget': sum_df['recommended budget'].sum(), 'share_%': 100.0}
    ...
    st.download_button('📄 Download Result (CSV)', data=csv_bytes, file_name='media_split_result_v5.csv', mime='text/csv')
    ...
    xls.seek(0)
    st.download_button('⬇️ Download Result (Excel)', data=xls.getvalue(), file_name='media_split_result_v5.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
