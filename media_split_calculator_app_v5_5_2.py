...
def ensure_mode():
    ...
    st.session_state.setdefault('last_calculated_total_budget', None)
...
def render_card(title, body_fn):
    ...
    Important: we do not use nested containers here so that the HTML
    <div class="msc-card"> really wraps all inner widgets (sliders,
    inputs, etc.). This gives the solid card blocks from the reference
    design.
    """
...
def do_recalc():
    ...
    total_budget_val = float(st.session_state.get('total_budget_cache', 240.0))
    ...
    st.session_state.last_calculated_total_budget = total_budget_val
...
if st.session_state.mode == 'filters':
    ...
    if df_result is None or summary is None:
        ...
        # Total should include FREE FLOAT even though that row is hidden
        # from the placements table above.
        total_all = pd.to_numeric(df_result['recommended budget'], errors='coerce').fillna(0).sum()
        total_row['placement'] = 'ИТОГО'
        total_row['recommended budget'] = float(total_all)
    ...
    if uploaded is not None:
        try:
            ...
            if total_from_summary is not None and total_from_summary>0:
                ...
                st.session_state['last_calculated_total_budget'] = float(total_from_summary)
            else:
                # Fallback: treat loaded sum as dataset total
                st.session_state['last_calculated_total_budget'] = float(total_loaded)
    ...
    with c1:
        if st.button('🔄 Save & Recalculate'):
            ...
            # Use the same total budget as the original dataset (calculation
            # or uploaded file), not the current slider value.
            tb_edit = st.session_state.get('last_calculated_total_budget')
            if tb_edit is None:
                tb_edit = float(st.session_state.get('total_budget_cache', 240.0))
            df_res, summary, total_margin, meta = allocate_with_manual_overrides(
                df_in,
                locked_map,
                total_budget=float(tb_edit),
                alpha=float(st.session_state.get('alpha_cache', 1.6)),
                beta=float(st.session_state.get('beta_cache', 1.0)),
                other_share=float(st.session_state.get('other_share_cache', 10.0)),
            )
            ...
            st.session_state.summary = summary.copy()
            st.session_state.total_margin = float(total_margin)
            st.session_state.last_calculated_total_budget = float(tb_edit)
