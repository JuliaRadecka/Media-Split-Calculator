...
def apply_platform_bounds(df, bounds):
    ...
    We distribute group min/max only across rows, которые реально участвуют
    в основном распределении (main_eligible), чтобы гарантировать выполнение
    ограничений по платформе в рамках main-бюджета.
    """
    df = add_platform_key(df)
    df = df.copy()
    ...
    # Маска строк, участвующих в основном распределении (как в allocate_budget):
    other_mask = df['category'].astype(str).map(normalize_category_key) == 'other'

    has_cat_prio = 'category priority' in df.columns and df['category priority'].notna().any()
    has_plc_prio = 'placement priority' in df.columns and df['placement priority'].notna().any()
    use_gates = bool(has_cat_prio and has_plc_prio)

    if use_gates:
        cat_prio = pd.to_numeric(df['category priority'], errors='coerce')
        plc_prio = pd.to_numeric(df['placement priority'], errors='coerce')
        cat_gate = cat_prio <= 3
        free_gate = cat_prio >= 4
        gate_mask = ((cat_gate & (plc_prio <= 2)) | free_gate)
    else:
        gate_mask = pd.Series(True, index=df.index)

    main_eligible = (~other_mask) & gate_mask
    ...
    for key, mm in (bounds or {}).items():
        ...
        # базовая маска по platform_key
        if canon is not None:
            mask_platform = pk == canon.lower()
        else:
            mask_platform = pk.str.contains(key_str, na=False)

        # плюс расширенный поиск по подстроке в raw placement
        mask_platform = mask_platform | pl.str.contains(key_str, na=False)

        # ограничиваемся только строками, реально участвующими в основном распределении
        mask = mask_platform & main_eligible
