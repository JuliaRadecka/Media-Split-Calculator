...
# -------------------- UI --------------------
st.set_page_config(page_title='Media Split Calculator v5.1', layout='wide')
st.title('📊 Media Split Calculator — v5.1 (fixed18)')
    ...
    if (base is None or (hasattr(base, 'empty') and base.empty)) and (current is None or current.empty):
        ...
        with c1:
            if st.button('✅ Apply Edits (Recalculate)'):
                ...
                # Optional: keep manual RB only if checkbox is ON
                if use_manual_rb and 'recommended budget' in edited.columns:
