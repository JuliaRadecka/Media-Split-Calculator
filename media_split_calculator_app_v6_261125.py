...
def apply_platform_bounds(df, bounds):
    ...
    * Здесь мы:
    ...
    # - сначала пробуем scenario_total_budget;
    # - если он None или отсутствует — берём total_budget_cache;
    # - в крайнем случае сваливаемся в 0.0, чтобы не падать с float(None).
    ...
    for key, mm in (bounds or {}).items():
        ...
        # Сопоставляем с каталогом и собираем предупреждения при конфликтах
        ...
        # финальная проверка: min не должен превышать max
        if plat_min > plat_max and plat_max > 0:
            warnings.append(
                f"❗Диапазон бюджета по платформе {platform_label} скорректирован до {plat_max:.2f} млн ₽ "
                f"из-за ограничений каталога или общего бюджета."
            )
            plat_min = plat_max
