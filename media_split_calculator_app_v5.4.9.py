...
# -*- coding: utf-8 -*-
# Media Split Calculator — v5.4.9
# Changes over v5.4.8:
# 1) Upload (Edit): hardened against any scalar/Series corner cases.
#    - All ".fillna(...)" are applied to guaranteed Series via seriesize().
#    - Blue banner % uses file Summary total when available; also syncs the Total Budget slider.
# 2) "Eye" behavior in Result: advanced columns are available in the Show/Hide menu WITHOUT a toggle.
#    We render all columns but set column_order to base ones so advanced are listed unchecked in the menu.
...
def seriesize(obj, length_hint=0, dtype='float64'):
    """Return a pandas Series for any input (Series/scalar/None)."""
...
# ---- App ----
st.set_page_config(page_title='📊 Media Split Calculator v5.4.9', layout='wide')
st.title('📊 Media Split Calculator — v5.4.9')
...
if st.session_state.mode == 'filters':
    ...
    if df_result is None or summary is None:
        ...
        # Render all columns, but order so that base visible; other columns available via the "eye" menu.
        ...
        column_order = base_cols  # advanced columns won't be shown by default, but will be available in the menu
    ...
    if uploaded is not None:
        try:
            ...
            rb_series = seriesize(base_df['recommended budget'] if 'recommended budget' in base_df.columns else pd.Series(dtype='float64'), len(base_df))
            rb_series = pd.to_numeric(rb_series, errors='coerce').fillna(0)
