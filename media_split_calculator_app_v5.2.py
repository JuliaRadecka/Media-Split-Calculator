    ...
    if (base is None or (hasattr(base, 'empty') and base.empty)) and (current is None or current.empty):
        ...
        with c1:
            if st.button('✅ Apply Edits (Recalculate)'):
                ...
                # Если пользователь вручную отредактировал recommended budget в редакторе — учитываем эти правки как final
                if 'recommended budget' in edited.columns:
                    import pandas as pd
                    overrides = edited[['placement','recommended budget']].copy()
                    overrides['recommended budget'] = pd.to_numeric(overrides['recommended budget'], errors='coerce')
        ...
        if st.session_state.get('last_edit_shown') and st.session_state.get('total_margin') is not None:
            ...
            try:
                ...
                with pd.ExcelWriter(_xls2, engine='openpyxl') as writer:
                    edited.to_excel(writer, index=False, sheet_name='Edited table')
            _xls2.seek(0)
            st.download_button('💾 Download edited table (Excel)', data=_xls2.getvalue(), file_name='edited_table.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', key='dl_edit_excel')
    ...
    if st.session_state.get('use_cached_result'):
        ...
        with dl2:
            ...
            st.download_button('💾 Download Result (Excel)',
                               data=xls.getvalue(),
                               file_name='media_split_result_v5.xlsx'mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', key='dl_calc_xlsx')
...
if 'df_result' in locals():
    ...
    with dl1:
        st.download_button('💾 Download Result (CSV)',
                           data=csv_bytes,
                           file_name='media_split_result_v5.csv',
                           mime='text/csv', key='dl_calc_csv_top')
    with dl2:
        xls = BytesIO()
        try:
            with pd.ExcelWriter(xls, engine='xlsxwriter') as writer:
        ...
        st.download_button('💾 Download Result (Excel)',
                           ...
                           mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', key='dl_calc_xlsx_top')
    with dl3:
        if st.button('✏️ Edit Calculated Table'):
