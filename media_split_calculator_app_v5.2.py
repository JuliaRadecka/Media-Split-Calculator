...
if st.session_state.mode == 'edit_result':
    ...
    if (base is None or (hasattr(base, 'empty') and base.empty)) and (current is None or current.empty):
        ...
        # Download current editable table as Excel
        ...
        with c1:
            if st.button('✅ ✅ Save & Recalculate'):
                ...
                # Apply manual overrides for 'recommended budget' ONLY where changed by user
                ...
                if rb_baseline is not None and 'recommended budget' in edited.columns:
                    ...
                    mask = cmp['recommended budget_new'].notna() & ((cmp['recommended budget_old'].isna()) | (abs(cmp['recommended budget_new'] - cmp['recommended budget_old']) > 1e-6))
                    overrides = cmp.loc[mask, ['placement','recommended budget_new']].rename(columns={'recommended budget_new':'recommended budget'})
                ...
                # Обновляем колонку 'recommended budget' в виде редактора по результатам пересчёта
                ...
                # Сохраняем обновлённую таблицу для повторного отображения
        ...
        if st.session_state.get('last_edit_shown') and st.session_state.get('total_margin') is not None:
            ...
            # Download edited table as Excel, shown below the margin
            from io import BytesIO
            _xls2 = BytesIO()
            try:
                with pd.ExcelWriter(_xls2, engine='xlsxwriter') as writer:
                    edited.to_excel(writer, index=False, sheet_name='Edited table')
            except ModuleNotFoundError:
                with pd.ExcelWriter(_xls2, engine='openpyxl') as writer:
                    edited.to_excel(writer, index=False, sheet_name='Edited table')
            _xls2.seek(0)
            st.download_button('💾 Download Result (Excel)', data=_xls2.getvalue(), file_name='edited_table.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', key='dl_edit_excel')
    ...
    if st.session_state.get('use_cached_result'):
        ...
        if 'df_result' in locals():
            ...
            dl1, sp1, dl2, sp2, dl3 = st.columns([1, 0.01, 1, 0.01, 1])
            with dl1:
...
if st.button('✏️ Edit Calculated Table'):
    ...
    if 'df_result' in st.session_state and 'recommended budget' in st.session_state.df_result.columns:
        st.session_state.rb_baseline = st.session_state.df_result[['placement','recommended budget']].copy()
...
with dl3:
    pass
# Move the edit button to the next row (full-width)
st.markdown('')
if st.button('✏️ Edit Calculated Table'):
    st.session_state.df_result = df_result.copy()
    st.session_state.summary = summary.copy()
    st.session_state.total_margin = total_margin
    # Save baseline recommended budget to detect manual overrides
    if 'recommended budget' in st.session_state.df_result.columns:
        st.session_state.rb_baseline = st.session_state.df_result[['placement','recommended budget']].copy()
    st.session_state.mode = 'edit_result'