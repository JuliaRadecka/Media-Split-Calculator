...
# ---- App ----
st.set_page_config(page_title='📊 Media Split Calculator v5.5.1', layout='wide')
...
if st.session_state.mode != 'edit':
    ...
    if st.session_state.get("show_optional_params", False):
        with st.container():
            ...
            # Category Priorities — three levels
            st.markdown("**Category Priorities — optional**")
            ui_groups = list(UI_GROUPS.keys())

            def _render_priority_level(title, level_key, color_emoji):
                st.markdown(f"{color_emoji} **{title}**")
                cols = st.columns(len(ui_groups))
                current_level = st.session_state.get(level_key, []) or []
                for idx, group in enumerate(ui_groups):
                    in_level = group in current_level
                    # проверяем, находится ли группа в других уровнях
                    in_other = False
                    for other_key in ["cat_level1", "cat_level2", "cat_level3"]:
                        if other_key != level_key and group in st.session_state.get(other_key, []):
                            in_other = True
                            break
                    if in_level:
                        label = f"● {group}"
                        ...
                        label = f"□ {group}"
                    if cols[idx].button(label, key=f"{level_key}_{group}", use_container_width=True):
                        lvl1 = st.session_state.get("cat_level1", []) or []
                        lvl2 = st.session_state.get("cat_level2", []) or []
                        lvl3 = st.session_state.get("cat_level3", []) or []
                        if in_level:
                            # снимаем выбор
                            new_level = [g for g in current_level if g != group]
                            st.session_state[level_key] = new_level
                        else:
                            # удаляем из других уровней и добавляем в текущий
                            lvl1 = [g for g in lvl1 if g != group]
                            lvl2 = [g for g in lvl2 if g != group]
                            lvl3 = [g for g in lvl3 if g != group]
                            new_level = current_level + [group]
                            if level_key == "cat_level1":
                                st.session_state["cat_level1"] = new_level
                                st.session_state["cat_level2"] = lvl2
                                st.session_state["cat_level3"] = lvl3
                            elif level_key == "cat_level2":
                                st.session_state["cat_level1"] = lvl1
                                st.session_state["cat_level2"] = new_level
                                st.session_state["cat_level3"] = lvl3
                            else:
                                st.session_state["cat_level1"] = lvl1
                                st.session_state["cat_level2"] = lvl2
                                st.session_state["cat_level3"] = new_level
                        # также обновляем cat_order для совместимости (Level1→2→3)
                        combined = (
                            st.session_state.get("cat_level1", [])
                            + st.session_state.get("cat_level2", [])
                            + st.session_state.get("cat_level3", [])
                        )
                        seen = set()
                        ordered = []
                        for g in combined:
                            if g not in seen:
                                seen.add(g)
                                ordered.append(g)
                        st.session_state["cat_order"] = ordered
                        mark_for_recalc()

            _render_priority_level("Priority Level 1", "cat_level1", "🟣")
            _render_priority_level("Priority Level 2", "cat_level2", "🔵")
            _render_priority_level("Priority Level 3", "cat_level3", "🟢")
