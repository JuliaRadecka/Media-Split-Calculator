...
# -*- coding: utf-8 -*-
# Media Split Calculator — v5.4.2
# Fix: replace deprecated st.experimental_rerun() with st.rerun() for broader Streamlit compatibility.
# Behavior:
#  • Filters mode: only Calculate button triggers compute
#  • Result mode: optional filters auto-recalc via flag
#  • No st.rerun() inside on_change callbacks; only in button flows where mode changes
...
# --- App ---
st.set_page_config(page_title='📊 Media Split Calculator v5.4.2', layout='wide')
st.title('📊 Media Split Calculator — v5.4.2')
...
def mark_for_recalc():
    st.session_state._pending_recalc = True  # callbacks only set the flag
